...
while res(step) > res_max && step < step_max
    step = step + 1; 
    [q_new,p] = FSM(q,p,q_prev,nu,nx,ny,np,nq,iu,iv,ip,dx,dy,dt, ...
    uR,vR,uL,vL,uT,vT,uB,vB);
...
save("temp_sol_"+nx+"_"+Re+".mat","Re","T","res","q","p","step","q_prev")
%%---------------------------------------------------------------%%
...
function [q_new,p_new] = FSM(q,p,q_prev,nu,nx,ny,np,nq,iu,iv,ip,dx, ...
    ...
    uT,vT,uB,vB,q);
    % 2nd step
    % use p as initial guess for p_new
    p_new = FSM_2(qF,p,nx,ny,iu,iv,ip,nq,np,dx,dy,uR,uL,vT,vB,dt,nu);
    % 3rd step
    q_new = FSM_3(qF,p_new,nx,ny,nq,ip,iu,iv,dx,dy,dt,nu);
end

%% calculate qF, 1st step of FSM
...
%% calculate P, 2nd step of FSM
function x = FSM_2(qF,p_ini,nx,ny,iu,iv,ip,nq,np,dx,dy,uR,uL,vT,vB,dt,nu)
    i = 0;
    eps = 1e-6;
    imax = np*50;
    b = FSM_2_b(qF,nx,ny,iu,iv,ip,np,dx,dy,uR,uL,vT,vB,dt);
    x = p_ini; %initial guess
    r = b-FSM_2_A(x,nx,ny,nq,np,ip,iu,iv,dx,dy,dt,nu); %r = b-A*x_ini
    d = r;
    delta_new = r.'*r;
    delta_0 = delta_new;
    
    while i<imax && delta_new>eps^2*delta_0
        Q = FSM_2_A(d,nx,ny,nq,np,ip,iu,iv,dx,dy,dt,nu);
        alpha = delta_new/(d.'*Q);
        x = x+alpha*d;
        r = r-alpha*Q;
        delta_old = delta_new;
...
%% 3rd step of FSM
function out = FSM_3(qF,p,nx,ny,nq,ip,iu,iv,dx,dy,dt,nu)
    G = grad(p,nx,ny,nq,ip,iu,iv,dx,dy);
    RiG = R_inv(G,nx,ny,iu,iv,nq,dx,dy,dt,nu);
    out = qF - dt*RiG;
end
%% FSM-related operators
%%% R (A of 1st step)
...
%%% b of 1st step
function out = FSM_1_b(q,q_prev,nx,ny,iu,iv,nq,dx,dy,uR,vR,uL,vL,uT, ...
    vT,uB,vB,dt,nu)
    L = lap(q,nx,ny,iu,iv,nq,dx,dy);
    S = q+dt/2*nu*L;
...
%%% A of 2nd step
function out = FSM_2_A(p,nx,ny,nq,np,ip,iu,iv,dx,dy,dt,nu)
    G = grad(p,nx,ny,nq,ip,iu,iv,dx,dy);
    RiG = R_inv(G,nx,ny,iu,iv,nq,dx,dy,dt,nu);
    out = div(RiG,nx,ny,iu,iv,ip,np,dx,dy); 
end
%%% b of 2nd step
function out = FSM_2_b(qF,nx,ny,iu,iv,ip,np,dx,dy,uR,uL,vT,vB,dt)
    D = div(qF,nx,ny,iu,iv,ip,np,dx,dy);
...
%% Initialization
function [q,p] = init(nx,ny,nq,np,iu,iv,ip)
    q = zeros(nq,1);
    p = zeros(np,1);
    for j = 1:ny
        for i = 2:nx

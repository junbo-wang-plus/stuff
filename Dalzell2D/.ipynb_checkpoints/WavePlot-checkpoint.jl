...
function plot_spectrum(option, params, output_dir, plot_size)
    spectrum_type = get(params, :spectrum_type, "gaussian")
    k_p = params[:k_p]
    k_w = params[:k_w]
    Hs = params[:Hs]
...
function generate_plots(option, batch, config_file=nothing)
    ...
    plot_spectrum(option, params, output_dir, plot_size)
